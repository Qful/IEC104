...
struct sMmsGooseControlBlock {
    ...
    LinkedList dataSetValues;					// значени€ данных в датасетах
 ...
 * функци€ активации режима симул€ции гусов
 *******************************************************/
 ...
 * функци€ сверки времени дл€ отправки и непосредственной отправки гусов.
 *******************************************************/
void	MmsGooseControlBlock_checkAndPublish(MmsGooseControlBlock self, uint64_t currentTime)
{
    if (currentTime >= self->nextPublishTime) {
        ...
        // отправка гуса
        GoosePublisher_publish(self->publisher, self->dataSetValues);			// self->dataSetValues значени€ данных в датасетах
