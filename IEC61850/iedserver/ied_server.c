...
#endif
extern uint32_t	GLOBALMemoryUsedLim;							//максимально использованной пам€ти
...
{
    DataSet* dataSet = self->model->dataSets;				// ¬от они датасеты. ¬ статической модели.
...
/*************************************************************************
 * IedServer_create
 * надо иметь уникальное им€ модели. будем добавл€ть к текущему [numb], последнее число IP адреса
 *************************************************************************/
IedServer
IedServer_create(IedModel* iedModel, uint16_t	numb)
{
    ...
    self->model = iedModel;
// -----------------------------------------------------------------------------
// тут ещЄ можно в модель чтото добавл€ть, дальше модель перет€нетс€ в структуру MMS и всЄ.
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// добавим блоки управлени€ отчЄтами
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
	...
	// забирает 200 б оперативы
	USART_TRACE("MmsMapping_create start\n");
    self->mmsMapping = MmsMapping_create(iedModel);							// тут перет€нули из IED модели в MMs структуры. ƒальше добавление в iedModel бессмысленно

    self->mmsDevice = MmsMapping_getMmsDeviceModel(self->mmsMapping);

    // забирает <1 б оперативы
	USART_TRACE("IsoServer_create start\n");
    self->isoServer = IsoServer_create();

    // забирает 17 б оперативы
	...
	// функциональные св€зи, которым разрешена запись из MMS
...
//    filesystem_Read_NamedVariableListRequest(self->mmsDevice, iedModel);
    ...
    // добавл€ем датасеты в модель (mmsDevice->mmsDomain->MmsNamedVariableList) из статической структуры iedModel
    createDataSets(self->mmsDevice, iedModel);
    //USART_TRACE_BLUE("создали не удал€емые DataSets из стат. модели\n");
 ...
 * отправка гусов и отчетов если эта функци€ активна дл€ данного атрибута данных
 *
 * проверка triggerOptions разрешЄн ли тригер
...
int	IedServer_updateFloatAttributeValueWithTime(IedServer self, DataAttribute* dataAttribute,DataAttribute* dataAttributeTime, uint64_t time, float value){
    ...
    {
    	IedServer_updateUTCTimeAttributeValue(iedServer, dataAttributeTime, time);		// обновим врем€
...
//void
// возврвщает true если были изменени€ в данных
int
...
if (oldQuality != (uint32_t) qual) {
   	IedServer_updateUTCTimeAttributeValue(iedServer, dataAttributeTime, time);		// сначала обновим врем€
...
int	IedServer_DataUpdateInGoosesDatasets(IedServer self)
{
    ...
    while ((element = LinkedList_getNext(element)) != NULL) {				// берЄм все гусы(gseControls) по списку.
        MmsGooseControlBlock gcb = (MmsGooseControlBlock) element->data;

        if (MmsGooseControlBlock_isEnabled(gcb)) {							// если этот гус активен
            DataSet* dataSet = MmsGooseControlBlock_getDataSet(gcb);		// берЄм датасет из Ѕ” гуса
        ...
        //дл€ каждого гуса будем решать надо отправка или нет
