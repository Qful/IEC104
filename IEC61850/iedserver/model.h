...
struct sDataObject {
	...
	int observerCount; 					// количество наблюдателей в текущий момент смотрят за этим узлом
...
struct sDataAttribute {
	...
	FunctionalConstraint fc;			// Функциональные связи
...
typedef struct {
	...
	char* dataSetName; 					// 14.2.2.6 Атрибут DatSet ссылка набора данных /* pre loaded with relative name in logical node */

	uint32_t confRef;    				// 14.2.2.7 Атрибут ConfRev ревизия конфигурации /* ConfRef - configuration revision */
	uint8_t trgOps;      				/* TrgOps - trigger conditions */
	uint8_t options;     				// 14.2.2.8 Атрибут OptFlds опциональные поля, включаемые в отчет /* OptFlds */
	uint32_t bufferTime; 				// 14.2.2.9 Атрибут BufTm буферное время /* BufTm - time to buffer events until a report is generated */
