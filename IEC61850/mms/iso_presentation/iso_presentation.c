...
//TODO: читай стандарт ISO 8823 зачем этот блок
	// в примере ту ещё 0x88 0x02 0x06 0x00
...
IsoPresentation_createUserData(IsoPresentation* self, ByteBuffer* writeBuffer, ByteBuffer* payload)
{
	...
	bufPos = BerEncoder_encodeTL(0x61, presentationLength, buffer, bufPos);				// размер блока presentation

	bufPos = BerEncoder_encodeTL(0x30, pdvListLength, buffer, bufPos);					// размер блока pdvList

	buffer[bufPos++] = (uint8_t) 0x02;
	buffer[bufPos++] = (uint8_t) 0x01;
	buffer[bufPos++] = (uint8_t) 3; /* mms context id */

	bufPos = BerEncoder_encodeTL(0xa0, payloadLength, buffer, bufPos);					// размер блока payload

	ByteBuffer_setcurrPos(writeBuffer, bufPos);

	ByteBuffer_append(writeBuffer, ByteBuffer_getBuffer(payload), payloadLength);			// дополним writeBuffer из payload размером payloadLength
 ...
 * парсим протокол представления ISO 8823 (ANS.1)
 *
 * self->nextContextId = presentationSelector
 *
 * указатель на результат ложим в
