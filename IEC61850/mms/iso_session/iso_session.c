...
parseAcceptParameters(IsoSession* session, ByteBuffer* message, int startOffset, int parameterLength)
{
	...
	while (offset < maxOffset) {
		...
		case 19: /* Protocol options */
			...
			USART_TRACE("iso_session: Param - Protocol Options: %02x\n", session->protocolOptions);
			hasProtocolOptions = 1;
			break;
		case 21: /* TSDU Maximum Size */
			USART_TRACE("iso_session: Param - TODO TSDU Maximum Size\n", param_val);
			offset += 4;
			break;
		case 22: /* Version Number */
			param_val = message->buffer[offset++];
			USART_TRACE("iso_session: Param - Version number: %02x\n", param_val);
			if (param_val != 2)
				return -1;
			hasProtocolVersion = 1;
			break;
		case 23: /* Initial Serial Number */
			USART_TRACE("iso_session: Param - TODO Initial Serial Number\n");
			offset += param_len;
			break;
		case 26: /* Token Setting Item */
			param_val = message->buffer[offset++];
			USART_TRACE("iso_session: Param - Token Setting Item: %02x\n", param_val);
			break;
		case 55: /* Second Initial Serial Number */
			USART_TRACE("iso_session: Param - TODO Second Initial Serial Number\n");
			offset += param_len;
			break;
		case 56: /* Upper Limit Serial Number */
			USART_TRACE("iso_session: Param - TODO Upper Limit Serial Number\n");
			offset += param_len;
			break;
		case 57: /* Large Initial Serial Number */
			USART_TRACE("iso_session: Param - TODO Large Initial Serial Number\n");
			offset += param_len;
			break;
		case 58: /* Large Second Initial Serial Number */
			USART_TRACE("iso_session: Param - TODO Large Second Initial Serial Number\n");
			offset += param_len;
			break;
		default:
			USART_TRACE("iso_session: Param - Invalid Parameter with ID %02x\n", pi);
			break;
		}
	}
...
parseSessionHeaderParameters(IsoSession* session, ByteBuffer* message, int parametersOctets)
{
	...
	while (offset < (parametersOctets + 2)) {
		...
		case 1:		/* Connection Identifier */
			USART_TRACE("iso_session: PGI - connection identifier\n");
			USART_TRACE("TODO: PGI not implemented!");
			offset += parameterLength;
			break;
		case 5:		/* Connection/Accept Item */
			USART_TRACE("iso_session: PGI - Connection/Accept Item\n");
		...
		case 20: /* Session User Requirements */
			USART_TRACE("iso_session: Parameter - Session User Req\n");
		...
		case 51: /* Calling Session Selector */
			USART_TRACE("iso_session: Parameter - Calling Session Selector\n");
			if (parameterLength != 2)
				return SESSION_ERROR;
		...
		case 52: /* Called Session Selector */
			USART_TRACE("iso_session: Parameter - Called Session Selector\n");
			if (parameterLength != 2)
				return SESSION_ERROR;
		...
		case 60: /* Data Overflow */
			USART_TRACE("iso_session: Parameter - Data Overflow\n");
			USART_TRACE("TODO: parameter not implemented!");
			offset += parameterLength;
			break;
		case 193:	/* User Data */
			USART_TRACE("iso_session: PGI - user data\n");
			...
			if (hasConnectAcceptItem && hasSessionRequirement)
			{
				ByteBuffer_wrap(&session->userData, message->buffer + offset,
						message->currPos - offset, message->maxSize - offset);
		...
		case 194:	/* Extended User Data */
			USART_TRACE("iso_session: PGI - extended user data\n");
			break;
		default:
			USART_TRACE("iso_session: invalid parameter/PGI\n");
			break;
		}
	}
...
IsoSession_createAcceptSpdu(IsoSession* self, ByteBuffer* buffer, uint8_t payloadLength)
{
	...
	buf[offset++] = 14; /* ACCEPT SPDU */				// тип SPDU
	lengthOffset = offset; offset++;

	offset = encodeConnectAcceptItem(buf, offset, self->protocolOptions);	// Connect Accept Item			пишем в buf начиная с offset с инкрементом

	offset = encodeSessionRequirement(self, buf, offset);					// Session Requirement

	offset = encodeCalledSessionSelector(self, buf, offset);				// Calling Session Selector

	offset = encodeSessionUserData(self, buf, offset, payloadLength);		// Called Session Selector
...
IsoSessionIndication		IsoSession_parseMessage(IsoSession* session, ByteBuffer* message)
{
	...
	case 14: /* ACCEPT SPDU */
		...
		if (parseSessionHeaderParameters(session, message, length) == SESSION_OK)
			...
			USART_TRACE("iso_session.c: error parsing accept spdu\n");
			return SESSION_ERROR;
		}
