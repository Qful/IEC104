...
#define BACKLOG 			5			// ожидающие подключение.
...
addClientConnection(IsoServer self, IsoConnection connection)
{
    ...
    if (DEBUG_ISO_SERVER) {
    	USART_TRACE_GREEN("IsoServer: »нкрементим счетчик клиентов до %i!\n", self->connectionCounter);
    }
    ...
    for (i = 0; i < CONFIG_MAXIMUM_TCP_CLIENT_CONNECTIONS; i++) {
        if (self->openClientConnections[i] != NULL) {
            if (IsoConnection_isRunning(self->openClientConnections[i])) {
                ...
                IsoConnection_handleTcpConnection(self->openClientConnections[i]);			// если это соединение уже открыто, то парсим и отвечаем. —юда надо попадать не только когда коннектитс€ новый клиент.
            }
            else {
...
handleIsoConnectionsThreadless(IsoServer self)
{
    ...
    if ((connectionSocket = ServerSocket_accept((ServerSocket) self->serverSocket)) != NULL) {				// было ли подключение к серверу? если да то вернем сокет дл€ текущего подключени€
        ...
        if (private_IsoServer_getConnectionCounter(self) >= CONFIG_MAXIMUM_TCP_CLIENT_CONNECTIONS) {		// если число клиентов превышает. то закроем соединение сразу.
            if (DEBUG_ISO_SERVER){
            	USART_TRACE_RED("ISO_SERVER: ƒостигли максимума клиентов, отключаем его.\n");
            }
        ...
        USART_TRACE("—оздадим соединение. ¬ыделим пам€ть дл€ него.\n");
        ...
        self->connectionHandler(ISO_CONNECTION_OPENED, self->connectionHandlerParameter, isoConnection);	// ¬ызываем isoConnectionIndicationHandler(); дл€ Iso соединени€
        																									// isoConnectionIndicationHandler(IsoConnectionIndication indication, void* parameter, IsoConnection connection);
    }
    handleClientConnections(self);				// ѕарсим если открыто. тут же даЄт ответ в порт, если сюда не попали то и ответа не будет.
}
 ...
 * Ќастройка и старт прослушивани€ сокета.
 *****************************************************************************************************/
void
...
IsoServer_waitReady(IsoServer self, unsigned int timeoutMs)
{
   ...
   if (self->state == ISO_SVR_STATE_RUNNING) {
       ...
       if (handles != NULL) {
           ...
           result = Handleset_waitReady(handles, timeoutMs);			// проверка состо€ни€ сокетов
