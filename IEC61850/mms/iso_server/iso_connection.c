 ...
 * парсим прин€тый COTP
 *
 *****************************************************************************************************/
//TODO: сделать монитор состо€ни€, и при отключении удал€ть соединение.
...
//	USART_TRACE_GREEN("число дескрипторов с данными %i сокет %i\n",(unsigned int)statwait,(unsigned int)GetSocket_num(self->socket));
    ...
    case COTP_CONNECT_INDICATION:
        if (DEBUG_ISO_SERVER){
        	USART_TRACE_BLUE("ISO_SERVER: COTP connection indication\n");
        }
    ...
    case COTP_DATA_INDICATION:
        {
            ...
            case SESSION_CONNECT:
                ...
                if (IsoPresentation_parseConnect(self->presentation, sessionUserData)) {
                    ...
                    if (aIndication == ACSE_ASSOCIATE) {
                        ...
                        if (DEBUG_ISO_SERVER){
                        	USART_TRACE_RED("ISO_SERVER: iso_connection: acse association failed. ISO_CON_STATE_STOPPED.\n");
                        }
                        self->state = ISO_CON_STATE_STOPPED;
                    }
            ...
            case SESSION_FINISH:
                if (DEBUG_ISO_SERVER){
                	USART_TRACE_BLUE("ISO_SERVER: iso_connection: session finish indication\n");
                }
                ...
                USART_TRACE_RED("SESSION_FINISH. ISO_CON_STATE_STOPPED.\n");
            ...
            case SESSION_ABORT:
                if (DEBUG_ISO_SERVER){
                	USART_TRACE_RED("ISO_SERVER: iso_connection: session abort indication. ISO_CON_STATE_STOPPED.\n");
                }
                self->state = ISO_CON_STATE_STOPPED;
                break;
            ...
            case SESSION_ERROR:
                if (DEBUG_ISO_SERVER){
                	USART_TRACE_RED("ISO_SERVER: iso_connection: session error indication. ISO_CON_STATE_STOPPED.\n");
                }
                self->state = ISO_CON_STATE_STOPPED;
                break;
            ...
            default: /* illegal state */
                if (DEBUG_ISO_SERVER){
                	USART_TRACE_RED("ISO_SERVER: iso_connection: session illegal state. ISO_CON_STATE_STOPPED.\n");
                }
    ...
    case COTP_ERROR:
        if (DEBUG_ISO_SERVER){
        	USART_TRACE_RED("ISO_SERVER: Connection closed. ISO_CON_STATE_STOPPED.\n");
        }
        self->state = ISO_CON_STATE_STOPPED;
        break;
    default:
        if (DEBUG_ISO_SERVER){
            printf("ISO_SERVER: COTP unknown indication. ISO_CON_STATE_STOPPED.  %i\n", cotpIndication);
        }
        self->state = ISO_CON_STATE_STOPPED;
        break;
    }
...
#if (CONFIG_MMS_SINGLE_THREADED == 0)
static void		handleTcpConnection(void* parameter)
{
