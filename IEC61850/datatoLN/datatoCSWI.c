...
/*******************************************************
 * Set_CSWI наполняем оперативными данными
 *******************************************************/
...
// Mod Beh
			...
			if ((ucConfigBufSW[MB_offset_ConfigSW+MB_rOffsetControlSW] & MB_bControlSW_ON_OFF)>0) {				// если введён выключатель
				Mod = STVALINT32_ON;

				if ((ucConfigBufSW[MB_offset_ConfigSW+MB_rOffsetControlSW] & MB_bControlSW_SDTU)>0) {			// если отключено управление от СДТУ
...
int		Set_CSWI	(uint16_t QTnum, uint64_t currentTime )
{
	...
	//  Mod Beh
				if ((ucOtherUstavkiInBuf[MB_offset_ConfigSW + MB_rOffsetControlSW] & MB_bControlSW_SDTU)>0) {				// если отключено управление от СДТУ
					Mod = STVALINT32_ON;
					ctlModel = CTLMODELEDENUM_DirectWithNormalSecurity;
				} else {
					Mod = STVALINT32_OFF;
