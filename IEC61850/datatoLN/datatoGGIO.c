...
int	Set_AVRGGIO	(uint16_t QTnum, uint64_t currentTime )
{
	...
	//  Mod Beh
		...
		BLKAVR = ucMUstavkiInBuf[MB_Addr_ConfigAWR + MB_offset_InBlok];						// номер дискрета
		if (BLKAVR) {
...
/*******************************************************
 * Set_NBLKGGIO наполняем оперативными данными
 *******************************************************/
 ...
 * дискреты модули: 1( 2шт) - Ind57-58
 * 					2(24шт) - Ind1-24
 * 					3(16шт) - Ind25-Ind40
 * 					4(16шт) - Ind41-Ind56
 *******************************************************/
...
// Качество и время меняем по надобности ---------------------------------------
	...
	bool		Q,T;
	uint16_t	num = QTnum & 0b0000111111111111;			// номер элемента
...
// Mod Beh -----------------------------------------------------------------------
	...
	for (i = 0; i < MB_MaxNumbDiscreet; i++) {
		if ((num == 0) || (num == i+1)){
			// GOOSE отсылается сразу, надо бы проверить весь датасет сначала сказать функции не отсылать пока не разрешу
...
/*******************************************************
 * Set_RSTGGIO наполняем оперативными данными
 *******************************************************/
...
#if defined (MR801) && (_REVISION_DEVICE<=299)		//2.07		ошибка в приборе. не верно выставляет этот флаг
		Mod	= STVALINT32_ON;
#endif
...
int	Set_VZGGIO	(uint16_t QTnum, uint64_t currentTime )
{
	...
	//  Quality (Mod Beh)
...
#if  defined (T20N4D40R35)
				if (IedServer_updateBooleanAttributeValue(iedServer, &iedModel_PROT_VZGGIO17_Alm_stVal,  ucMDiscInBuf[MB_offset_SRAB_ext_17] & MB_b_SRAB_ext_17))		// СРАБ
				IedServer_updateUTCTimeAttributeValue(iedServer, &iedModel_PROT_VZGGIO17_Alm_t, currentTime);

				if (IedServer_updateBooleanAttributeValue(iedServer, &iedModel_PROT_VZGGIO18_Alm_stVal,  ucMDiscInBuf[MB_offset_SRAB_ext_18] & MB_b_SRAB_ext_18))		// СРАБ
				IedServer_updateUTCTimeAttributeValue(iedServer, &iedModel_PROT_VZGGIO18_Alm_t, currentTime);

				if (IedServer_updateBooleanAttributeValue(iedServer, &iedModel_PROT_VZGGIO19_Alm_stVal,  ucMDiscInBuf[MB_offset_SRAB_ext_19] & MB_b_SRAB_ext_19))		// СРАБ
				IedServer_updateUTCTimeAttributeValue(iedServer, &iedModel_PROT_VZGGIO19_Alm_t, currentTime);

				if (IedServer_updateBooleanAttributeValue(iedServer, &iedModel_PROT_VZGGIO20_Alm_stVal,  ucMDiscInBuf[MB_offset_SRAB_ext_20] & MB_b_SRAB_ext_20))		// СРАБ
	 ...
	 defined (T24N0D40R35)
				...
				if (IedServer_updateBooleanAttributeValue(iedServer, &iedModel_PROT_VZGGIO21_Alm_stVal,  ucMDiscInBuf[MB_offset_SRAB_ext_21] & MB_b_SRAB_ext_21))		// СРАБ
				IedServer_updateUTCTimeAttributeValue(iedServer, &iedModel_PROT_VZGGIO21_Alm_t, currentTime);

				if (IedServer_updateBooleanAttributeValue(iedServer, &iedModel_PROT_VZGGIO22_Alm_stVal,  ucMDiscInBuf[MB_offset_SRAB_ext_22] & MB_b_SRAB_ext_22))		// СРАБ
				IedServer_updateUTCTimeAttributeValue(iedServer, &iedModel_PROT_VZGGIO22_Alm_t, currentTime);

				if (IedServer_updateBooleanAttributeValue(iedServer, &iedModel_PROT_VZGGIO23_Alm_stVal,  ucMDiscInBuf[MB_offset_SRAB_ext_23] & MB_b_SRAB_ext_23))		// СРАБ
				IedServer_updateUTCTimeAttributeValue(iedServer, &iedModel_PROT_VZGGIO23_Alm_t, currentTime);

				if (IedServer_updateBooleanAttributeValue(iedServer, &iedModel_PROT_VZGGIO24_Alm_stVal,  ucMDiscInBuf[MB_offset_SRAB_ext_24] & MB_b_SRAB_ext_24))		// СРАБ
...
#if defined (MR901) && (_REVISION_DEVICE<=212)		//2.06		ошибка в приборе. не верно выставляет этот флаг
	Mod	= STVALINT32_ON;
#endif
