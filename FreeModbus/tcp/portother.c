...
/* ----------------------- Defines ------------------------------------------*/
#define MB_FRAME_LOG_BUFSIZE    250//512
...
void	prvvMBTCPLogFrame( const CHAR * pucMsg, UCHAR * pucFrame, USHORT usFrameLen )
{
    ...
    res = snprintf( &arcBuffer[iBufPos], iBufLeft, "Len = %u ", usFrameLen );
    iBufPos += res;
    iBufLeft -= res;
    ...
    for( i = 0; i < usFrameLen; i++ )
    {
        ...
        /* выводим сами данные Print the data. */
...
void	vMBPortLog( eMBPortLogLevel eLevel, const CHAR * szModule, const CHAR * szFmt, ... )
{
    ...
    USART_TRACE( "%s: %s: ", arszLevel2Str[eLevel], szModule );
    va_start( args, szFmt );
    vprintf( szFmt, args );
    va_end( args );
}
