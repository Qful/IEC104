...
#if (CONFIG_CS104_SUPPORT_TLS == 1)	// Протокол защиты транспортного уровня TLS-протокол основан на спецификации протокола SSL версии 3.0
CS104_Slave
...
/*******************************************************************
 * По умолчанию сервер прослушивает все локальные IP-адреса.
 * С помощью функции CS104_Slave_setLocalAddress можно ограничить сервер одним локальным IP-адресом.
 ********************************************************************/
...
/*******************************************************************
 * Порт TCP по умолчанию для IEC 60870-5-104 - 2404. Порт можно изменить с помощью функции CS104_Slave_setLocalPort.
 ********************************************************************/
...
/*************************************************
 * MasterConnection_destroy
 * удаляем сокет self->socket
 * освобождаем self->sentASDUs
 * удаляем семафор self->sentASDUsLock
 * освобождаем MasterConnection структуру
 *************************************************/
static void		MasterConnection_destroy(MasterConnection self)
{
    if (self) {
...
/***********************************************************
 * CS104_Slave_removeConnection
 * удаляем соединени не удаляем ТАСК
 ***********************************************************/
static void		CS104_Slave_removeConnection(CS104_Slave self, MasterConnection connection)
{
    ...
    // удаляем соединение для клиента. но не закрываем ТАСК
...
connectionHandlingThread(void* parameter)
{
    ...
    while (self->isRunning) {
        ...
        if (isAsduWaiting)
            ...
            socketTimeout = 20; /*100 TODO replace by configurable parameter */
    ...
    // удаляем соединение из списка и освобождаем память от self. Но не завершаем ТАСК
 ...
 *******************************************/
static MasterConnection		MasterConnection_create(CS104_Slave slave, Socket socket, MessageQueue lowPrioQueue, HighPriorityASDUQueue highPrioQueue)
{
    ...
    if (self != NULL) {
        ...
        // создали задачу для клиента.
        Thread newThread = Thread_create((ThreadExecutionFunction) connectionHandlingThread, (void*) self, true);

        //Thread_start(newThread);
        // нужно дать разные имена на всякий случай. добавим номер сокета socket.fd GetSocket_num(socket);
        char	name[] = "104Con ";
        name[6] = '0'+GetSocket_num(socket);
        Thread_start(newThread,IEC870Task__PRIORITY,IEC104Con_STACK_SIZE,name);
...
/********************************************************************************
 * serverThread
 * принимаем новое соединение и создаём для него отдельный таск для обработки
 * добавляем новое соединение в список self->masterConnections
 *******************************************************************************/
    ...
    while (self->stopRunning == false) {
        ...
        if (newSocket != NULL) {
            ...
            if (acceptConnection && (self->connectionRequestHandler != NULL)) {
                ...
                acceptConnection = self->connectionRequestHandler(self->connectionRequestHandlerParameter, ipAddress);
            }
            ...
            if (acceptConnection) {
                ...
                MasterConnection connection = MasterConnection_create(self, newSocket, lowPrioQueue, highPrioQueue);
...
CS104_Slave_enqueueASDU(CS104_Slave self, CS101_ASDU asdu)
{
    ...
    if (self->serverMode == CS104_MODE_CONNECTION_IS_REDUNDANCY_GROUP) {
        ...
        /************************************************
		 * отправка всем открытым клиентским подключениям
         ************************************************/
             ...
             element = LinkedList_getNext(element))
        {
            MasterConnection connection = (MasterConnection) LinkedList_getData(element);
            if (connection)
            	MessageQueue_enqueueASDU(connection->lowPrioQueue, asdu);			// зависает на ожидании семафора при отключении клиента.
        }
...
/*******************************************************
 * Эта функция запускает новый фоновый поток, который прослушивает входящие клиентские соединения.
 *******************************************************/
...
#endif
        self->listeningThread = Thread_create(serverThread, (void*) self, false);				// задача слушает порт и принимает новые соединения

        //Thread_start(self->listeningThread);
        Thread_start(self->listeningThread,IEC870Task__PRIORITY,IEC870_STACK_SIZE,"104Lst");
...
/*******************************************************
 * Для деактивации службы IEC 60870-5-104 сервер может быть остановлен функцией CS104_Slave_stop.
 *******************************************************/
