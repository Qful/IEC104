...
extern FATFS SPIFLASHDISKFatFs;  	//FATFS filesystem;
...
void StartTFTPTask(void const * argument){
  ...
  ServerSocket_listen((ServerSocket) IsoServer_GetTFTPServerSocket(isoServer));				// Начинаем слушать входящие подключения

	for(;;)
	{
		  if ((connectionSocketTFTP = ServerSocket_accept((ServerSocket) IsoServer_GetTFTPServerSocket(isoServer))) != NULL) {

			  	  USART_TRACE_GREEN("соединение на TFTP порт.\n");

			  	  int buflen = 500;
			  	  unsigned char recv_buffer[buflen];

			  	  // читаем из сокета запрос от клиента
			  	  read(GetSocket_num(connectionSocketTFTP), recv_buffer, buflen);

//		         http_server_serve(connectionSocketTFTP);
		      } else{
                        ...
                        struct ip_addr *addr, u16_t port)
{
  ...
  process_tftp_request(pkt_buf, addr, port);				// буфер данных, адрес клиента, порт клиента
...
void process_tftp_request(struct pbuf *pkt_buf, struct ip_addr *addr, u16_t port)
{
  ...
  switch (op)
  {
    case TFTP_RRQ: /* TFTP RRQ (read request) */
    {
      ...
      if(f_mount(&SPIFLASHDISKFatFs, (TCHAR const*)"", 0) != FR_OK)
      {
        return;
      }
