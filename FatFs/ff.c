...
)
{
	...
	*rfs = 0;
	vol = get_ldnumber(path);					// номер логического диска
...
#endif
	if (res == FR_OK) {
//		USART_TRACE("Нашел таблицу FAT , n_rootdir=%u\n",dj.fs->n_rootdir);
//		USART_TRACE("Ищем файл..... %s\n",path);
		INIT_BUF(dj);
		// тут коцается n_rootdir
		res = follow_path(&dj, path);	/* Follow the file path */
//		USART_TRACE("переходим в папку с файлом... %s, n_rootdir=%u \n",path,dj.fs->n_rootdir);
		dir = dj.dir;
		...
		if (mode & (FA_CREATE_ALWAYS | FA_OPEN_ALWAYS | FA_CREATE_NEW)) {
			...
			if (res != FR_OK) {					/* No file, create new */
				if (res == FR_NO_FILE)			/* There is no file to open, create a new entry */
					...
					if (enq_lock()){
//    	 			  	USART_TRACE_RED("Такой файл не нашли. Создадим новый. n_rootdir=%u\n",dj.fs->n_rootdir);
						res = dir_register(&dj);
					}else{
...
/***************************************************************************
 *
 * Формат FDISK обычно используется для жестких дисков HDD, MMC, SDC и CFC.
 * Он может использоваться для деления физического диска на один или большее
 * количество разделов, с созданием таблицы разделов в MBR (Master Boot Record).
 *
 * Формат SFD является методом без использования разделов диска.
 ***************************************************************************/
FRESULT f_mkfs (
	const TCHAR* path,	/* Логический номер диска */
	BYTE sfd,			/* Привило разделения на разделы 0:FDISK, 1:SFD */
	UINT au				/* Размер кластера (allocation unit) [bytes] */
)
{
	...
	if (_MULTI_PARTITION && part) {
		...
		if (sfd) {	/* No partition table (SFD) */
			...
			//CHS адрес первого блока в разделе. Формат описан в следующих 3 байтах.
			tbl[1] = 1;						// Partition start head начинается раздел
			tbl[2] = 1;						// Дорожка, на которой начинается раздел (биты 16-6),
			tbl[3] = 0;						// и сектор, на котором начинается раздел (биты 5-0).

			//Тип раздела
			tbl[4] = sys;					//  (1h - FAT12, 4h - FAT16<32Мб, 6h - FAT16>32Мб, Eh - FAT16>??Мб, Bh,Ch - FAT32)

			//CHS адрес последнего блока в разделе. Формат описан в следующих 3 байтах.
			tbl[5] = 254;					// Partition end head заканчивается раздел
			n = (b_vol + n_vol) / 63 / 255;	// общее число секторов делим зачемто  на 83 и на 256
			tbl[6] = (BYTE)(n >> 2 | 63);	// Дорожка, на которой заканчивается раздел (биты 16-6),
			tbl[7] = (BYTE)n;				// и сектор, на котором заканчивается раздел (биты 5-0).
//			n = (b_vol + n_vol);
//			tbl[6] = (BYTE)(n>>8);
//			tbl[7] = (BYTE)n;

			// LBA
			ST_DWORD(tbl+8, 63);			// 8-11 Partition start in LBA
			ST_DWORD(tbl+12, n_vol);		// 12-15 Partition size in LBA

			ST_WORD(fs->win.d8+BS_55AA, 0xAA55);	// MBR signature	адрес 1FEh	Сигнатура (55h AAh)
